...
export class ConfigValidationService {
  ...
  private validateDatabaseConfig(): void {
    ...
    if (isNaN(port) || port < 1 || port > 65535) {
      throw new Error(`❌ 数据库端口无效: DB_PORT 必须是 1-65535 之间的有效端口，当前值: ${portStr}`);
    }
  ...
  private validateRedisConfig(): void {
    ...
    const port = Number.parseInt(portStr, 10);
    if (isNaN(port) || port < 1 || port > 65535) {
      throw new Error(`❌ Redis 端口无效: REDIS_PORT 必须是 1-65535 之间的有效端口，当前值: ${portStr}`);
    }
    ...
    if (isNaN(db) || db < 0 || db > 15) {
      throw new Error(`❌ Redis 数据库编号无效: REDIS_DB 必须是 0-15 之间的数字，当前值: ${dbStr}`);
    }
  ...
  private validateMinioConfig(): void {
    const requiredFields = ['MINIO_ENDPOINT', 'MINIO_ACCESS_KEY', 'MINIO_SECRET_KEY', 'MINIO_BUCKET'];
    ...
    if (accessKey.length < 3) {
      throw new Error(`❌ MinIO Access Key 无效: 长度必须至少为 3 个字符，当前长度: ${accessKey.length}`);
    }

    // 验证 MINIO_SECRET_KEY 长度（MinIO 要求至少 8 个字符）
    const secretKey = this.configService.get<string>('MINIO_SECRET_KEY')!;
    if (secretKey.length < 8) {
      throw new Error(`❌ MinIO Secret Key 无效: 长度必须至少为 8 个字符，当前长度: ${secretKey.length}`);
    }
    ...
    const port = Number.parseInt(portStr, 10);
    if (isNaN(port) || port < 1 || port > 65535) {
      throw new Error(`❌ MinIO 端口无效: MINIO_PORT 必须是 1-65535 之间的有效端口，当前值: ${portStr}`);
    }

    // 验证 MINIO_USE_SSL
    const useSslStr = this.configService.get<string>('MINIO_USE_SSL', 'false');
    if (!['true', 'false'].includes(useSslStr.toLowerCase())) {
      throw new Error(`❌ MinIO SSL 配置无效: MINIO_USE_SSL 必须是 'true' 或 'false'，当前值: ${useSslStr}`);
    }
  ...
  private validateSecureIdConfig(): void {
    ...
    try {
      ...
      throw new Error(`❌ SECURE_ID_SECRET_KEY 格式无效: 无法解析为 hex 或 base64 格式`);
    }
    ...
    if (this.isWeakKey(secretKey)) {
      throw new Error(`❌ SECURE_ID_SECRET_KEY 安全风险: 检测到弱密钥或示例密钥，请使用强随机密钥。生成命令: openssl rand -hex 32`);
    }

    this.logger.log(`✅ 安全 ID 配置验证通过 (密钥长度: ${keyBuffer.length} 字节)`);
  }
  ...
  private validateAuthConfig(): void {
    ...
    const expiresStr = this.configService.get<string>('AUTH_TOKEN_EXPIRES_IN', '3600');
    const expires = Number.parseInt(expiresStr, 10);
    if (isNaN(expires) || expires < 60 || expires > 86400) {
      throw new Error(`❌ Token 过期时间无效: AUTH_TOKEN_EXPIRES_IN 必须是 60-86400 秒之间的数字，当前值: ${expiresStr}`);
    }

    // 验证 AUTH_TOKEN_BYTES_LENGTH
    const bytesStr = this.configService.get<string>('AUTH_TOKEN_BYTES_LENGTH', '32');
    const bytes = Number.parseInt(bytesStr, 10);
    if (isNaN(bytes) || bytes < 16 || bytes > 64) {
      throw new Error(`❌ Token 字节长度无效: AUTH_TOKEN_BYTES_LENGTH 必须是 16-64 之间的数字，当前值: ${bytesStr}`);
    }

    // 验证 AUTH_BCRYPT_ROUNDS
    const roundsStr = this.configService.get<string>('AUTH_BCRYPT_ROUNDS', '10');
    const rounds = Number.parseInt(roundsStr, 10);
    if (isNaN(rounds) || rounds < 10 || rounds > 15) {
      throw new Error(`❌ bcrypt 轮数无效: AUTH_BCRYPT_ROUNDS 必须是 10-15 之间的数字，当前值: ${roundsStr}`);
    }

    // 验证 AUTH_MAX_LOGIN_ATTEMPTS
    const attemptsStr = this.configService.get<string>('AUTH_MAX_LOGIN_ATTEMPTS', '5');
    const attempts = Number.parseInt(attemptsStr, 10);
    if (isNaN(attempts) || attempts < 1 || attempts > 20) {
      throw new Error(`❌ 最大登录尝试次数无效: AUTH_MAX_LOGIN_ATTEMPTS 必须是 1-20 之间的数字，当前值: ${attemptsStr}`);
    }

    // 验证 AUTH_LOCKOUT_TIME
    const lockoutStr = this.configService.get<string>('AUTH_LOCKOUT_TIME', '900');
    const lockout = Number.parseInt(lockoutStr, 10);
    if (isNaN(lockout) || lockout < 60 || lockout > 3600) {
      throw new Error(`❌ 锁定时间无效: AUTH_LOCKOUT_TIME 必须是 60-3600 秒之间的数字，当前值: ${lockoutStr}`);
    }
